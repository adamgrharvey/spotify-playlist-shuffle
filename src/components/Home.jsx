...
export default function Home(props) {
  ...
  const [colorTarget, setColorTarget] = useState(
    'https://i.scdn.co/image/ab67616d0000b2735c133c0318f05040ef9d16b5'
  )
  ...
  const REDIRECT_URL_AFTER_LOGIN = process.env.REACT_APP_REDIRECT_URL_AFTER_LOGIN ?? 'ERROR'
  ...
  useEffect(() => {
    if (tokenRefresh) {
      if (expiresIn === 0) {
        Authentication(setExpiresIn, setTokenRefresh)
      } else {
  ...
  }, [userPlaylists, currUser.id, userAuth])
    ...
    <>
      <div className="App">
        {colorPalette !== undefined ? (
          <div
            className="backgroundColorChanger"
            style={{
              'background-color': `rgb(${colorPalette[0][0]}, ${colorPalette[0][1]}, ${colorPalette[0][2]})`,
            }}
          ></div>
        ) : (
          <div
        ...
        <a target="_blank" href="https://github.com/adamgrharvey" rel='noreferrer'>
          <div className="appSubTitle">by Adam Harvey</div>
        </a>
        ...
        {!currPlaylist.data.id && userAuth.access_token ? (
          <button
        ...
        {!currPlaylist.data.id && userAuth.access_token && (
          <div className="descText">Select a Playlist and start scrambling!</div>
        )}
        ...
        {!currPlaylist.data.id ? (
          <div className="list">
            {!currPlaylist.data.id &&
              !userPlaylists.end &&
              userPlaylists.next == null &&
              userPlaylists.items.map((playlist, i) => (
                <div
                  className="playlistDiv"
                  onMouseOver={() => {
                    setColorTarget(playlist.images[0].url)
                  }}
                  onClick={() => {
                    getPlaylistData(userAuth, setCurrPlaylist, playlist.id, false)
                  }}
                  key={i}
                >
                  <div className="playlistImage">
                    {playlist.images[0] && (
                      <img height={64} src={playlist.images[0].url} alt="playlistImage1"></img>
                    )}
                  </div>
          ...
          <div>
            ...
            <div>
              {JSON.stringify(currPlaylist.tracks) ===
              JSON.stringify(currPlaylist.shuffleTracks) ? (
                <button disabled={true} onClick={() => resetTracks(setCurrPlaylist)}>
                  Undo
                </button>
              ) : (
                <button disabled={false} onClick={() => resetTracks(setCurrPlaylist)}>
                  Undo
                </button>
              )}
              <button onClick={() => shuffleTracks(currPlaylist.shuffleTracks, setCurrPlaylist)}>
                Shuffle
              </button>
              {JSON.stringify(currPlaylist.tracks) === JSON.stringify(currPlaylist.shuffleTracks) ? (
                <button disabled={true}>{saveButtonTxt}</button>
              ) : (
                <LoadingButton
                  loading={saving}
                  loadingIndicator={
