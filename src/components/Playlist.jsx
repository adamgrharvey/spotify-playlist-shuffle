  ...
  return (
    <>
      <div className="PlaylistTitleImage">
        <div>
          <img
            width={200}
            height={200}
            src={currPlaylist.data.images[0].url}
            alt="PlaylistImage"
          ></img>
        </div>
        <div>
          <div
            className={`PlaylistTitleLarge-${htmlDecode(currPlaylist.data.name).length > 16 ? `over11char` : `under12char`}`}
          >
            {htmlDecode(currPlaylist.data.name)}
          </div>
          ...
          <div className="PlaylistDetails">
            ...
            <span>
              {currPlaylist.data.followers.total > 0 ? (
                <span className="DotSpace">{`• ${currPlaylist.data.followers.total} likes `}</span>
              ) : (
                ``
              )}
            </span>
            <span className="DotSpace">{`• ${currPlaylist.tracks.length} songs, `}</span>
            <span className="PlaylistDuration">{countPlaylistDuration(currPlaylist)}</span>
          </div>
        </div>
      </div>
      <div className="contentSpacing">
        <div
          role="grid"
          aria-rowcount={currPlaylist.tracks.length + 1}
          aria-colcount={AriaCol}
          aria-label={`${currPlaylist.data.name}`}
          className="PlaylistGrid WidthToggle"
          tabIndex="0"
        >
          <div className="HeaderStyle" style={{ top: '0px' }} role="presentation">
            <div className="PlaylistHeader GridSizing" role="row" aria-rowindex="1">
              <div
                className="PlaylistIndexSpacing"
                role="columnheader"
                aria-colindex="1"
                aria-sort="none"
                tabIndex="-1"
              ...
              >
                <div
                  aria-label="Duration"
                  className="FlexCenter DurationSpacing"
                  aria-expanded="false"
                >
                  <svg
                    role="img"
          ...
          <div className="" role="presentation">
            ...
            <div role="presentation" style={{ transform: 'translateY(0px)' }}>
              {currPlaylist.next === null &&
                currPlaylist.shuffleTracks.map((track, i) => (
